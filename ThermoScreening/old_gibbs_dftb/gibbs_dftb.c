/**
 * @file gibbs_dftb.c
 * @author Stefanie Kröll (S.Kroell@student.uibk.ac.at) and 
 * Josef M. Gallmetzer (Josef.Gallmetzer@student.uibk.ac.at)
 * @brief Thermochemistry program to calculate 
 * thermodynamically properties like Gibbs Free energy
 * @version 0.1
 * @date 2022-06-10
 *
 ...
 * group: T. Hofer (t.hofer@uibk.ac.at)
 */
...
/*** Define Prototype Functions for the external programms 
 * symmetry_i.c and atom_properties.c ***/

/**
 * symmetry_i.c (https://github.com/nquesada/symmetry): 
 * Taken from the xtb-source code
 ...
 * xtb is free software: you can redistribute it and/or modify 
 ...
 * See the GNU Lesser General Public License for more details.
 ...
 GNU Lesser General Public License along with xtb.  
...
/**
 * @brief Compute the symmetry of the system in Schoenflies 
 * notation by the external program symmetry_i.c
 * Use: int getsymnum()
 * 
 ...
 */
void schoenflies(int natoms, int *attype,
 double *coord, char *symbol, double *paramar);
 ...
 * @return int symmetry number
 */
int initialize_sigma(int natoms, double *coords, 
int *atom_types, int linear)
{ // Declaration and initialization of Schoenfliess symbol
   ...
   * https://github.com/grimme-lab/
   * xtb/blob/b9dbd81ecc5158898a213ee80afcc0b39a7efee9/src/thermo.f90
   */
...
/**
 * @brief Calculation of the Mass center position for the 
 * moment of inertia calculations later.
 * Use: It is used for the rotational component. 
 * Formula: r_c = {Sum_{i=0}^{numAtom} r_{i}S * mass_{i} } / M , r={x,y,z} 
 * Explaination: Sum over all products of atom coordinates 
 * and atom masses and then divided by system mass
 *
 ...
 * @param massCenter 3D-array to return the calculated mass 
 * center in Angstrom unit for x-,y-, z-direction
 * @param numAtoms number of atoms in system
 */

void massCenterPosition(double *mass, double *xAxis, 
double *yAxis, double *zAxis, double massCenter[3], int numAtoms)
{ 
...
void relocate(double *xAxis, double *yAxis, 
double *zAxis, double massCenter[3], int numAtoms)
{
  // Sum over all atoms
  for (size_t i = 0; i < (size_t)numAtoms; i++)
  {
 ...
 * Formula: I_{ii} = Sum_{i=0}^{numAtoms} mass_{i}*(iAxis^2 + jAxis_i^2),
 *  i is runtime variable over all atoms
 ...
 * Use: in void inertiaTensor() for rotational contribution 
 * Formula: I_{ij} = -Sum_{i=0}^{numAtoms} mass_{i}* iAxis_i * jAxis_i, 
 * i is runtime variable over all atoms
 * @param iAxis parameter which coordinate (x,y,z)
 * @param jAxis parameter which coordinate (x,y,z)
 * @param mass atom mass in Molar mass units
 ...
 * @return double
 */
double inertiaFunctionIJ(double *iAxis, 
double *jAxis, double *mass, int numAtoms)
{
  // Declaration and initialization
  // non-diagonal element the moment of inertia tensor
 ...
 */
void inertiaTensor(double *xAxis, double *yAxis, 
double *zAxis, double *mass, int numAtoms, double tensor[3][3])
{
  // Computation of all tensor elements by using void inertiaFunctionII() 
  //and void inertiaFunctionIJ()
...
void eigenValues(double tensor[3][3], double eigVals[3])
{
  ...
  */
   ...
   * D = q^3+r^2 
   * with q = (3*C - B^2)/9, r = (-(27*D) + B * (9*C - 2*B^2))) / 54
   * 
   */
   ...
   * z^3 + p*z + q = 0,
   * with p = (3*C - B^2 / 9 and q = (-(27*D) + B*(9*C - 2*B^2)) / 54)
...
void transl_contribution(double mSum, double *St, double *Et, double *Ct)
{
  ...
  *Ct = 3.0 / 2 * R / cal;             // in unit cal/ mol K
  ...
  // Check
  printf("qt: %.12lf, St: %.12lf cal/ mol K, Et:  
  .12lf H/Particle, Ct: %.12lf cal/ mol K\n\n", qt, *St, *Et, *Ct);
...
void elec_contribution(double *Se, double *Ee, double *Ce)
{
  ...
  *Ce = 0;
  ...
  // Check
  printf("qe: %.12lf, Se: %.12lf cal/ mol K, Ee:  
  %.12lf H/Particle, Ce: %.12lf cal/ mol K\n\n", qe, *Se, *Ee, *Ce);
  return;
}
...
int main(int argc, char *argv[]) 
{
  ...
  while (--argc > 0)
  {
    ...
    {
      if (!strcasecmp(argv[argc] + 9, "true") || 
      !strcasecmp(argv[argc] + 9, "t") || !strcasecmp(argv[argc] + 9, "1"))
      {
        symcalc = 1;
      }
    }
  ...
  while (fgets(line, sizeof(line), geometry) != NULL)
  {
    ...
    if (sscanf(line, "%s %lf %lf %lf", atomName,
     xAxis + i, yAxis + i, zAxis + i) != 4)
    {
  ...
  // Check: Geometry output after 
  //relocation of the coordinates to mass center
  ...
  rot_contribution(eigVal, &Sr, &Er, &Cr);
  ...
  Etot = (Er + Ev + Et + Ee); // in unit H/Particle
  Stot = (Sr + Sv + St + Se); // in unit cal/mol K
  Ctot = (Cr + Cv + Ct + Ce); // in unit cal/mol K
  printf("Etot: %.12lf H/Particle, 
    Stot: %.12lf cal/ mol K,Ctot: %.12lf cal/ mol K\n\n", Etot, Stot, Ctot);
  ...
  Hcorr = Etot + (R * T) / H / Na; // in unit H/Particle

  Gcorr = Hcorr - (Stot * cal / Na / H) * T; // in unit H/Particle
  ...
  if (electronic_energy == NULL)
  {
    ...
    fprintf(stderr, "\n(x) Electronic Energy file 
      (electronic_energy.txt) is not available!\n\n");
    return EXIT_FAILURE;
  }
  ...
  E0Etot = (E0 + Etot);         // Sum of electronic and thermal energies
  E0Hcorr = (E0 + Hcorr);       // Sum of electronic and thermal enthalpies
  E0Gcorr = (E0 + Gcorr);       // Sum of electronic and thermal Free Energies
