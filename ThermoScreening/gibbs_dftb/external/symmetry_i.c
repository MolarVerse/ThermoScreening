    ...
    for (i = 0; i < AtomsCount; i++)
    {
        ...
        if (k != i && elem->transform_atom == rotate_reflect_atom)
        {
            ...
            for (j = elem->order; j > 0; j--, k = elem->transform[k])
            {
                ...
                {
                    if (verbose > 0)
                        printf("        (improper) transform looped %d steps too early from atom %d\n", j, i);
                    return -1;
                }
            }
        }
...
int refine_symmetry_element(SYMMETRY_ELEMENT *elem, int build_table)
{
    ...
    if (build_table && (establish_pairs(elem) < 0))
    {
        StatPairs++;
        if (verbose > 0)
            printf("        no transformation correspondence table can be constructed\n");
        return -1;
    }
    ...
    if (check_transform_quality(elem) < 0)
    {
        StatOpt++;
        if (verbose > 0)
            printf("        refined transformation does not pass the numeric threshold\n");
        return -1;
    }
    StatAccept++;
    return 0;
...
init_higher_axis(int ia, int ib, int ic)
{
    ...
    {
        if (verbose > 0)
            printf("    no coherrent axis is defined by the points\n");
        return NULL;
    }
    axis->transform_atom = rotate_atom;
    if (refine_symmetry_element(axis, 1) < 0)
...
init_improper_axis(int ia, int ib, int ic)
{
    ...
    if (r <= ToleranceSame)
    {
        StatEarly++;
        if (verbose > 0)
            printf("    atoms can not define improper axis of the order more than 2\n");
        return NULL;
    }
    for (i = 0; i < DIMENSION; i++)
        centerpoint[i] /= r;
    ...
    {
        if (verbose > 0)
            printf("    no coherrent improper axis is defined by the points\n");
        return NULL;
    }
...
void report_symmetry_elements_brief(void)
{
    int i;
...
void report_symmetry_elements_brief_Conly(void)
{
    int i;
    char *symmetry_code = calloc(1, 10 * (PlanesCount + NormalAxesCount + ImproperAxesCount + InversionCentersCount + 2));
    char buf[100];
    ...
    else
    {
        for (i = MaxAxisOrder; i >= 2; i--)
        {
            ...
            {
                sprintf(buf, "C%d ", i);
                strcat(MaxRotAxis, buf);
            }
        }
    }
}

int identify_point_group(void)
{
    int i;
    int j;
    int last_matching = -1;
...
int getsymnum(char *pgroup, int *linear)
{
    ...
    if (strstr(pgroup, "ih") != NULL)
        symnum = 60;
    if (strstr(pgroup, "c") != NULL && linear)
        symnum = 1; //.and.lin
    if (strstr(pgroup, "d") != NULL && linear)
        symnum = 2; //.and.lin
    if (strstr(pgroup, "Ci") != NULL)
        symnum = 1;
    if (strstr(pgroup, "Cs") != NULL)
...
int old_main(int argc, char **argv)
{
    ...
    for (argc--, argv++; argc > 0; argc -= 2, argv += 2)
    {
        ...
        else if (strcmp(*argv, "-minchange") == 0)
        {
        ...
        else if (strcmp(*argv, "-maxoptstep") == 0)
        {
        ...
        else if (strcmp(*argv, "-minoptstep") == 0)
        {
        ...
        else if (strcmp(*argv, "-gradstep") == 0)
        {
        ...
        else if (strcmp(*argv, "-maxoptcycles") == 0)
        {
        ...
        else if (strcmp(*argv, "-maxaxisorder") == 0)
        {
