...
class TestThermo(unittest.TestCase):
    ...
    def test_thermo_aq_2(self):
        ...
        np.testing.assert_allclose(thermo._electronic_partition_function, 1.000000000000, atol=1e-10)

        np.testing.assert_allclose(thermo._electronic_entropy, 0.000000000000, atol=1e-10)

        np.testing.assert_allclose(thermo._electronic_heat_capacity, 0.000000000000, atol=1e-10)

        np.testing.assert_allclose(thermo._electronic_energy*4.814/(4.3597447222071 * 10**(-18))/(6.02214076 * 10**(23)), 0.000000000000, atol=1e-10)
        ...
        np.testing.assert_allclose(thermo.total_entropy("cal/(mol*K)"), 99.467891485529 , atol=1e-4) 

        np.testing.assert_allclose(thermo.total_gibbs_free_energy("H"), 0.137848485863 , atol=1e-3) 
        ...
        np.testing.assert_allclose(thermo.total_EeGtot(), -33.700859097137 , atol=1e-3) 
